...
Type inference has gotten more powerful as well: many type checkers can now infer return types or empty container types, which reduces the need for annotations.

With these fast and IDE native type checkers, you get much more than catching bugs. Python code is frequently object-oriented, so only a type analysis can easily track the identities of things across attribute access. This means types are key to getting:
...
Over the years many library maintainers have added types because they find types are a feature or help with maintainability. And typeshed maintains stubs for many of the other libraries; these get better over time:
- SQLAlchemy 2.0 rewrote with types in mind in 2023
- NumPy's 2025 type coverage jump: 33% → 88% in months
- From survey data: in 2025 91% of typing-aware developers use types "always" or "often"
...
- And AI can also help with one of the most vexing problems: migrating untyped code to have type annotations
...
- Types provide structured context about code at generation time
  - Code graphs like Glean and CodeQL can help AI explore large codebases or dependency trees in a structured way, and with more types they can index better
...
- Types serve as a kind of documentation of code that can be both programmatically verified and programmatically used to provide tools like IDE features.
  - This is part of why, in the early days, typing was most used by large companies
...
Python typing has made tremendous strides in usability and adoption in the last few years.
